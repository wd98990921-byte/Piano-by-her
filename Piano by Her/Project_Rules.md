...
- 当前想法是否偏离 Session 01《重新开始》真实用户测试。
- 当前想法是否增加了不必要复杂度。
...
当前 MVP 的 Medium 只使用钢琴。其他 Medium 记录在 Parking Lot，不进入当前开发。
